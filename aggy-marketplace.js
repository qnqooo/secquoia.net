(()=>{
  ...
  $('#aggyVoiceTalk')?.addEventListener('click',()=>{setPanel('chat');mic?.click();$('#aggyVoiceState').textContent='Micrófono solicitado. La transcripción se procesa con las capacidades del navegador.'});
  let recorder=null,recordingStream=null,voiceChunks=[],voiceNote=null;
  $('#aggyVoiceRecord')?.addEventListener('click',async event=>{const button=event.currentTarget,state=$('#aggyVoiceState');if(recorder?.state==='recording'){recorder.stop();button.querySelector('strong').textContent='Grabar nota';return}try{recordingStream=await navigator.mediaDevices.getUserMedia({audio:true});voiceChunks=[];voiceNote=null;recorder=new MediaRecorder(recordingStream);recorder.ondataavailable=data=>{if(data.data.size)voiceChunks.push(data.data)};recorder.onstop=()=>{voiceNote=new Blob(voiceChunks,{type:recorder.mimeType||'audio/webm'});recordingStream.getTracks().forEach(track=>track.stop());recordingStream=null;state.textContent=`Nota capturada en memoria · ${voiceNote.size} bytes · pendiente de QuSOC y E2EE/PQC`;button.querySelector('strong').textContent='Grabar nota'};recorder.start();button.querySelector('strong').textContent='Detener';state.textContent='Grabando en memoria temporal…'}catch{state.textContent='Micrófono no disponible o permiso denegado.'}});
  $('#aggyVoiceRead')?.addEventListener('click',()=>{const messages=$$('#conversation .msg.agent'),text=messages.at(-1)?.childNodes[0]?.textContent||messages.at(-1)?.textContent||'';if(!text||!('speechSynthesis'in window)){ $('#aggyVoiceState').textContent='No existe una respuesta o la síntesis de voz no está disponible.';return}const language=$('#aggyLanguage')?.value||'AUTO',locale={EN:'en-US',ES:'es-CO',FR:'fr-FR',DE:'de-DE',IT:'it-IT',PT:'pt-BR'}[language]||navigator.language||'es-CO',voices=speechSynthesis.getVoices(),voice=voices.filter(item=>item.lang?.toLowerCase().startsWith(locale.slice(0,2).toLowerCase())).sort((a,b)=>/natural|neural|premium/i.test(b.name)-/natural|neural|premium/i.test(a.name))[0],personality=$('#aggyPersonality')?.value||'friendly',delivery={friendly:[1.06,1.03],executive:[1.10,1.00],technical:[1.00,1.00],empathetic:[.99,1.02]}[personality]||[1.04,1.01],utterance=new SpeechSynthesisUtterance(text);utterance.lang=voice?.lang||locale;utterance.rate=delivery[0];utterance.pitch=delivery[1];if(voice)utterance.voice=voice;speechSynthesis.cancel();speechSynthesis.speak(utterance);$('#aggyVoiceState').textContent=voice?`Lectura fluida · ${voice.name} · ${voice.lang}`:`Voz básica del sistema · ${locale}`});
