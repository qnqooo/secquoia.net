...
test('Aggy exposes communication-first navigation and preserves advanced areas',()=>{
  ...
  for(const name of ['contacts','groups','calls','voice','files','models','security','more'])assert.match(html,new RegExp(`data-open-aggy-panel="${name}"`));
...
test('Aggy Realtime replaces browser legacy speech and recording',()=>{
  ...
  assert.match(html,/Aggy v1\.0\.0-rc\.41 [^<]* Voz de SQAILE - Acento neutro/);
  assert.match(html,/Voz de SQAILE - Acento neutro/);
  assert.match(html,/Leer con Aggy/);
  assert.match(bridge,/window\.AggyVoice\.start/);
});

test('Aggy keeps the composer clean and the three primary actions in the conversation header',()=>{
  ...
  assert.doesNotMatch(html,/id="aggyOpenAttachmentSheet"|id="aggyAttachmentSheet"/);
...
test('QuMarket add-ons expose the hybrid QuCFA commercial model',()=>{
  ...
  for(const credit of ['qvit-ai-credit-1','qvit-ai-credit-10','qvit-ai-credit-25','qvit-ai-credit-100','qvit-ai-credit-500'])assert.ok(addons.includes(`id:'${credit}'`));
