...
test('Aggy compact widget uses the governed Realtime voice client only',()=>{
  ...
  assert.match(widget,/src="\/aggy-realtime-voice\.js\?v=1\.0\.0-rc\.4"/);
...
test('SECQUOIA public pages load the local Aggy distribution',()=>{
  assert.match(index,/src="\/aggy-embed\.js\?v=1\.0\.0-rc\.4"[^>]*data-aggy-site="secquoia\.net"/);
  assert.match(notFound,/src="\/aggy-embed\.js\?v=1\.0\.0-rc\.4"[^>]*data-aggy-site="secquoia\.net"/);
});
